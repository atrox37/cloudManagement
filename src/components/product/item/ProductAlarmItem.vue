...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const addFunc = () => {
      dataCondition.push({
        dataCondition: property.value[0].id,
        operation: "=",
        value: "",
      });
          ...
          for (var i in dataCondition) {
            if (dataCondition[i].column == value) {
              ...
              if (dataCondition[i].valueType == "enum") {
                dataCondition[i].enumData =
                  property.value[index].valueType.extra.enumData;
              }
              break;
            }
          }
    ...
    const handlerConditionChange = (index) => {
            ...
            if (property.value[i].valueType.type == "number") {
              ...
              if (property.value[i].valueType.extra.enumData.length > 0) {
                dataCondition[j].value =
                  property.value[i].valueType.extra.enumData[0].key;
              } else {
                dataCondition[j].value = "";
              }
            } else {
      ...
      console.log(
        "handlerConditionChange:" + JSON.stringify(dataCondition[index])
      );
    };
