<template>
    <el-container>
        ...
        <el-main>
            <el-table height="100%" :data="tableData" v-loading="loading" stripe border @row-click="editClick">
                <el-table-column prop="templatePo.name" label="模板名称" width="200" header-align="center" align="center"/>
                <el-table-column prop="configPo.name" label="通知配置" width="150" header-align="center" align="center"/>
                <el-table-column label="模板类型" width="100"  header-align="center" align="center">
                  <template #default="scope">
                    <el-tag>{{notifyEnum(scope.row)}}</el-tag>
                  </template>
                </el-table-column>
                <el-table-column prop="sysUserPo.username" label="创建人" width="100"  header-align="center" align="center"/>
                <el-table-column prop="sysDimensionPo.name" label="所属机构" width="180"  header-align="center" align="center"/>
                <el-table-column prop="templatePo.createTime" label="创建时间" header-align="center" align="center"/>
                <el-table-column prop="templatePo.updateTime" label="更新时间" header-align="center" align="center"/>
                <el-table-column>
                    <template #header>
                        <div class="center-flex-contain">
                            <el-button-group>
                                <el-button @click.native.stop="addClick"><font-awesome-icon size="1x" :icon="['fasr', 'square-plus']" /></el-button>
                            </el-button-group>
                        </div>
                    </template>
                    <template #default="scope">
  ...
  <NotifyTemplateDialog :data="newNotifyTempate" :config="notifyConfigAll" @save="createClick"></NotifyTemplateDialog>
</template>
...
<script>
  ...
  export default defineComponent({
    ...
    components: {NotifyTemplateDialog},
    setup(){
      ...
      const newNotifyTempate=reactive({state:false,loading:false,data:{name:'',notifyId:undefined,msgType:1}})
      ...
      const createClick=(data)=>{
        ...
        proxy.$http.notifyTemplateUpdate(data).then(value=>{
      ...
      const editClick=(row)=>{
        ...
        router.push({
            ...
            query: {
                code: row.configPo.code,
                templateId: row.templatePo.id,
            }
        })
      }
