<template>
  <div v-if="lazyLoad" style="width: 100%;height:100%;padding: 0;margin: 0px">
    <ContentHeader>
      <template #more>
        <el-descriptions :title="deviceData.deviceInstancePo.name" :column="6"
          ...
          <el-descriptions-item span="4">
            ...
            <template #default>
              <el-tag :type="deviceStatusTag">
                {{ deviceData.deviceInstancePo.status == 'offline' ? '离线' : '在线' }}
              </el-tag>
            </template>
          </el-descriptions-item>
        </el-descriptions>
    ...
    <el-tabs v-model="activeName" class="tab-container" @tab-click="handleClick">
      <el-tab-pane label="基本信息" name="first">
        <DeviceDetail :deviceData="deviceData" @editClick="editDialogClick"></DeviceDetail>
      </el-tab-pane>
      <el-tab-pane label="模型属性" name="five">
        <DeviceMeta ref="deviceMetaRef" :deviceUnit="deviceUnit" :deviceMeta="deviceMeta"
                    @updateClick="updateDeviceInstanceApi"></DeviceMeta>
      </el-tab-pane>
      <el-tab-pane label="运行状态" name="second">
        <DeviceRun ref="deviceRunView"
                   :historyData="propertyHistoryData"
                   :deviceMeta="deviceData.deviceInstancePo"
                   @readProperty="readProperty"
                   @writeProperty="writeProperty"
                   @propertyClick="propertyDialogShow"></DeviceRun>
      </el-tab-pane>
      <el-tab-pane label="设备功能" name="third">
        <DeviceFunction ref="deviceFuncRef" :deviceMeta="deviceData.deviceInstancePo" :functionData="tabFunctionData"
                        @funcExecution="funcExecution"></DeviceFunction>
      </el-tab-pane>
      <el-tab-pane label="日志信息" name="fourth">
        <DeviceLog ref="deviceLogRef" :data="logData" @pageRequest="deviceLogApi"></DeviceLog>
      </el-tab-pane>
      <el-tab-pane label="设备告警" name="six">
        <DeviceAlarm ref="deviceAlarmRef" :deviceData="deviceData" @open="alarmOpen"></DeviceAlarm>
      </el-tab-pane>
      <el-tab-pane v-if="deviceData.productPo.type == 'gateway'" label="子设备" name="seven">
        <DeviceChildren ref="deviceChildrenRef" :deviceData="deviceData" @addChildrenClick="addChildrenClick"
                        @delChildrenClick="delChildrenClick" @updateMeta="updateDeviceInstanceApi"></DeviceChildren>
      </el-tab-pane>
    </el-tabs>
  </div>

  <div v-if="loading" v-loading="loading" style="background:#C0C4CC;width: 100%;height:100%;padding: 0;margin: 0;">

  </div>

  <DialogDeviceEdit :data="deviceData" :gateways="gatewayData" :status="dialogEdit" @save="updateDeviceInstanceBase"
                    @cancel="dialogEdit=false"></DialogDeviceEdit>
  <DialogAlarm :deviceData="deviceData" :alarmData="dialogAlarmData" :status="dialogAlarmState" @close="alarmClose"
               @reload="alarmReload"></DialogAlarm>
  <DialogProperty :status="dialogPropertyStatus"
                  :propertyData="dialogDataProperty"
                  :property="dialogProperty"
                  @close="propertyDialogCancel"
                  @propertyApi="queryDevicePropertyData">
  </DialogProperty>
  <DialogChildrenAdd ref="dialogChildrenRef" :data="dialogChildrenData" @submit="addChildrenSubmit"></DialogChildrenAdd>
  <DialogPropertyControl :data="dialogPropertyControl" @close="propertyControlCancel" @submit="propertyControlSubmit"></DialogPropertyControl>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const deviceTab = ref([{name: "设备名称", value: "测试设备"}, {name: "产品", value: "测试产品"}])
    ...
    const dialogChildrenData = reactive({status: false,treeNode:null, deviceData: deviceData.value})
    const propertyHistoryData = ref({})
    const logData = ref({})
    const dialogPropertyControl=reactive({state:{loading:false,dialog:false},meta:{}})
    const tabFunctionData=reactive({loading: false, result: {},resultStr:''})
    ...
    const connectToStomp = function () {
      ...
      stomp.connect({'Authorization': window.sessionStorage.getItem('token')}, function (frame) {
        console.log('Connected: ');
        stomp.subscribe(reportTopic, function (greeting) {
          console.log('report_property-->' + reportTopic + '--->' + greeting.body)
    ...
    const receive = function () {
      stomp.onreceive = function (m) {
        ...
        if(JSON.parse(m.body).replyType == 'TIMEOUT'&&JSON.parse(m.body).type=='report-property'){
        ...
        if(JSON.parse(m.body).replyType=='SUCCESS'&&JSON.parse(m.body).type=='report-property'){
        ...
        if(JSON.parse(m.body).replyType == 'TIMEOUT'&&JSON.parse(m.body).type=='write-reply'){
          propertyLoading(JSON.parse(m.body).requestWsData.property,'write',false)
          ElMessage({
            message: '写入超时',
            type: 'error',
            plain: true,
        ...
        if(JSON.parse(m.body).replyType=='FAIL'&&JSON.parse(m.body).type=='request-reply'){
          tabFunctionData.loading=false
          tabFunctionData.result = (JSON.parse(m.body).resultMapData==undefined?{}:JSON.parse(m.body).resultMapData)
          tabFunctionData.resultStr = JSON.parse(m.body).resultStrData
          ElMessage({
            message: '返回结果失败',
            type: 'error',
    ...
    const deviceLogApi = (page, terms) => {
      const t = [{column: 'device_id', value: deviceData.value.deviceInstancePo.id}]
      t.push(...terms)
      page.terms = t
    ...
    const funcExecution = function (funId, data) {
      ...
      const functionData = {type: 'request', deviceId: deviceId, function: funId, args: data}
      let functionDataStr = JSON.stringify(functionData)
      stomp.send('/queue/queue_stomp_request', {'reply-to': '/temp-queue/foo'}, functionDataStr);
    }
    const readExecution = function (property) {
      const readData = {type: 'read-property', deviceId: deviceId, property: [property]}
      let readDataStr = JSON.stringify(readData)
      console.log(readData)
      stomp.send('/queue/queue_stomp_read', {'reply-to': '/temp-queue/foo'}, readDataStr);
    }

    const writeExecution = function (p, v) {
      const writeData = {type: 'write-property', deviceId: deviceId, property: p, value: v}
      let writeDataStr = JSON.stringify(writeData)
      console.log(writeDataStr)
      stomp.send('/queue/queue_stomp_write', {'reply-to': '/temp-queue/foo'}, writeDataStr);
    }
    ...
    const queryDevicePropertyData = (page, terms) => {
      var termsData = [
        {column: 'product_id', value: deviceData.value.deviceInstancePo.productId},
    ...
    const delChildrenClick = (row) => {
      ...
      proxy.$http.updateDeviceInstanceApi({id: row.deviceInstancePo.id}).then(value => {
        console.log("修改成功")
        ElMessage({
          message: '操作成功',
          type: 'success',
