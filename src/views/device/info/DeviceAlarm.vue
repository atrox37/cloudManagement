<template>
    <div class="tab-pan-content">
        <el-main>
            <el-table height="100%" :data="pageData" v-loading="loading" stripe @row-click="rowClick" style="width: 100%">
                <el-table-column prop="id" label="规则ID" width="80" header-align="center" align="center"/>
                <el-table-column prop="name" label="规则名称" width="200" header-align="center" align="center"/>
                <el-table-column label="触发方式" width="200" header-align="center" align="center">
                    <template #default="scope">
                        {{scope.row.ruleData.type=='time'?'周期':'定时'}}
                    </template>
                </el-table-column>
                <el-table-column label="规则" width="200" header-align="center" align="center">
                    <template #default="scope">
                ...
                <el-table-column label="状态" width="80" header-align="center" align="center">
                    <template #default="scope">
                        <el-tag style="margin-left: 5px" type="warning">{{scope.row.state==1?'启动':'关闭'}}</el-tag>
                    </template>
                </el-table-column>
...
<script>
    import {defineComponent,watch, reactive, ref, getCurrentInstance, onMounted, toRef,defineExpose} from "vue"
    ...
    export default defineComponent({
        ...
        setup(props,context) {
            ...
            const loadApi=()=>{
                loading.value=true
                proxy.$http.deviceAlarmPage({current:pageIndex.value,size:pageSize.value,terms:[{column:"t.device_id",value:deviceInfo.value.id}]}).then(response=>{
            ...
            return {reloadApi,loading,deviceInfo,pageTotal,pageData,pageChange,rowClick,handlerCroe,loadApi}
        }
    })
</script>
<style scoped>
